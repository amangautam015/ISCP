...
def listCourses(request):
    department = Course.objects.order_by('title')
    if not department:
        return HttpResponse("<p>No Departments Registered Yet contact DC</p>")
    return render(request, 'interface/olist.html', {'department':department})
def listChapter(request):
    department = Chapter.objects.order_by('chapter_title')
    if not department:
        return HttpResponse("<p>No Departments Registered Yet contact DC</p>")
    return render(request, 'interface/clist.html', {'department':department})
def listDepartments(request):
    department = Department.objects.order_by('title')
    if not department:
        return HttpResponse("<p>No Departments Registered Yet contact DC</p>")
    return render(request, 'interface/dlist.html', {'department':department})
